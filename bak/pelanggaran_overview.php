    ...
    try {
        ...
        if ($num > 0) {
            while ($row = $result->fetch(PDO::FETCH_NUM)) {
                ...
                $str = $str . "<td valign='top'>" . $row[23] . "</td>"; //status

                $row[0] = base64_encode($row[0]);

                if ($row[23] == 'Draft' || $row[23] == '') {
                    ...
                    $str = $str . "<td class='center' width='150px'>
                                <a class='btn btn-success' onclick='return konfirmasiApprove()' id='link_approve' href='pelanggaran_approve.php?id=$row[0]&tgl=$row[2]&name=$row[16]&pj=$row[7]&pelanggaran=$row[14]&masa_berlaku=$row[20]&sp=$row[21]&email=$row[26]'>
                                    <i class='icon-edit icon-white'></i>  
                                    Approve                                            
                                </a>
                            </td>";
                    ...
                    $str = $str . "<td class='center' width='150px'>
                                        <a class='btn btn-success' onclick='return pelanggaranDownload()'>
                                            Download                                            
                                        </a>
                                    </td>
                                    <td class='center' width='150px'>
                                        <a class='btn btn-success' href='pelanggaran_approve.php?id=$row[0]&tgl=$row[2]&name=$row[16]&pj=$row[7]&pelanggaran=$row[14]&masa_berlaku=$row[20]&sp=$row[21]&email=$row[26]'>
                                            Resend email                                            
                                        </a>
                                    </td>
                                    <td class='center' width='150px'>
...
<body class="with-side-menu">
    ...
    <script>
        ...
        function pelanggaranDownload() {
            alert('Fitur dalam pengembangan')
        }

        function konfirmasiApprove() {
            // var myLink = document.getElementById("link_approve");
            // var myURL = myLink.href;
            // myURL = myURL.split('email=')[1];
            // var mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
            // if (myURL.match(mailformat)) {
            //     if (confirm('Anda yakin akan meng-approve? Catatan: Data yang sudah di-approve tidak dapat diedit kembali. Email akan dikirim ke Penanggung jawab, pastikan email penanggung jawab terisi!')) {
            //         alert('Data pelanggaran berhasil di-approve')
            //     } else {
            //         return false
            //     }
            // } else {
            //     alert("Alamat email penanggung jawab tidak valid! Mohon ubah di master data penanggung jawab");
            //     return false;
            // }
            // // myURL = myURL.substring(myURL.indexOf('email') + 1);
